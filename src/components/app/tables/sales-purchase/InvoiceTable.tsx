...
const InvoiceTable: React.FC<PurchaseOrderTableProps> = ({
  items,
  setItems,
  onTotalsChange = () => {},
  toggleAddItemModal,
  ...
  //     const price = parseFloat(currentItems[index].unitPrice || "0"); // Changed from price to unitPrice
  //     currentItems[index].totalPrice = (quantity * price).toFixed(2); // Changed from totalBeforeTax to totalPrice
  ...
  //     (sum, item) =>
  //       sum +
  //       (parseFloat(item.totalPrice || "0") * parseFloat(item.tax || "0")) /
  //         100,
  //     0
  //   );
  //   return {
  ...
  // SOLUTION 3: Memoized columns to prevent recreation - updated per image
  const columns: ColumnDef<Item>[] = useMemo(
    () => [
      {
        header: "Item Name",
        accessorKey: "itemId", // Changed from itemId to itemDescription
        cell: ({ row }) => {
          const idx = row.index;
          const selectedItem = allItems.find(
            (i) => i.id === row.original.item.id
          );
            ...
            <SelectWithSearch
              ...
              onChange={(id) => {
                ...
                const updated: Item = {
                  ...
                  hsn: found.hsnCode,
                  hsnSacCode: found.hsnCode, // For backwards compatibility
                  quantity: "0",
                  ...
                  item: {
                    ...
                    type: "",
                    currentStock: "0",
                    defaultPrice: found.defaultPrice,
          ...
          return (
            <div className="relative">
              <QuantityInput
                ...
                // onChange={(newQty) => updateItem(idx, { quantity: newQty })}
                disabled={true} // Disable input for now, can be enabled later
              />
      ...
      // },
      {
        ...
        cell: ({ row }) => {
          return (
            <div className="relative">
              <div className="font-normal min-w-32 text-sm">
                {row.original.item.currentStock}
              </div>
              <div className="absolute right-2 top-1/2 -translate-y-1/2">
                <svg
                  width="16"
      ...
      {
        header: "Price",
        accessorKey: "unitPrice", // Changed from price to unitPrice
        cell: ({ row }) => {
          const idx = row.index;
          return (
            <div className="flex items-center">
              <PriceInput
                value={parseFloat(itemsRef.current[idx].unitPrice || "0")}
                // onChange={(newPrice) =>
                //   updateItem(idx, { unitPrice: newPrice.toString() })
                // }
                disabled={true}
              />
      ...
      {
        header: "Total",
        accessorKey: "totalPrice", // Changed from totalBeforeTax to totalPrice
        cell: ({ row }) => {
          return (
            <div className="font-normal min-w-32 text-sm">
