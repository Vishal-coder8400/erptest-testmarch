  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        billingAddressId: buyerLocation.id,
        deliveryAddressId: deliveryLocation.id,
      };

      console.log([payload]);

      const data = await post(`/inventory/purchase-quotation`, payload);
      console.log("Order created:", data);

      if (data?.data?.status === "PENDING") {
        SuccessToast({
          title: "Purchase quotation submitted successfully.",
          description: "",
        });
        navigate("/sales-purchase?tab=purchase-quotations");
      } else {
        ErrorToast({
