...
const CreateSalesQuotation: React.FC = () => {
  ...
  const [isSupplierLocationModalOpen, setIsSupplierLocationModalOpen] =
  ...
  const [supplierLocation, setSupplierLocation] = useState<any>(
    JSON.parse(localStorage.getItem("selectedSupplierLocation") || "null")
  );
  ...
  const validateForm = (): boolean => {
    ...
    if (!supplierLocation) {
      newErrors.billingAddress = "Supplier location is required";
    }
    ...
    if (!kindAttention.trim()) {
      newErrors.kindAttention = "Kind attention is required";
    }
    if (!warehouseId) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const processedItems = items.map(({ itemId, ...rest }) => ({
        ...
        totalAmount: Number(rest.price) * Number(rest.quantity),
      ...
      const payload = {
        ...
        amendment,
        deliveryDate,
        customerInquiryNumber: enquiryNumber,
        customerInquiryDate: enquiryDate,
        paymentType: paymentTerms,
        kindAttention,
        warehouse: String(warehouseId),
        items: processedItems,
        billingAddressId: supplierLocation.id,
        deliveryAddressId: deliveryLocation.id,
      };

      const data = await post(`/inventory/sales-quotation?tab=sales-quotations`, payload);
      console.log("Order created:", data);

      if (data?.data?.status === "PENDING") {
        SuccessToast({
          title: "Sales quotation submitted successfully.",
          description: "",
        });
        navigate("/sales-purchase");
      } else {
        ErrorToast({
          title: "Failed to submit sales quotation",
          description: "",
        });
      }
    } catch (err: any) {
  ...
  const handleSelectSupplierLocation = (location: any) => {
    setSupplierLocation(location);
    setIsSupplierLocationModalOpen(false);
    localStorage.setItem("selectedSupplierLocation", JSON.stringify(location));
    ...
    <div className="pt-5 pb-16">
          ...
          <div className="border-[1.5px] h-[50%] rounded-lg border-neutral-200 text-sm">
            <div className="px-3 py-2 flex bg-[#F7F7F8] rounded-t-lg justify-between items-center gap-2">
              ...
              <Button
                variant="outline"
                className="border h-7 px-2 border-gray-400 font-normal"
                onClick={() => setIsSupplierLocationModalOpen(true)}
              >
                <img src="/icons/edit.svg" className="h-4 -mr-1" />
                Edit
              </Button>
            ...
            <div className="px-3 py-2 space-y-1">
              ...
              <div className="font-medium">
                {" "}
                Contact Person: {selectedSupplier?.name}{" "}
              </div>
              <div className="font-medium">
                {" "}
                Phone: +91 {selectedSupplier?.phone}{" "}
              </div>

              {supplierLocation ? (
                <div>
                  <p className="font-medium">{supplierLocation.locationName}</p>
                  <p>{supplierLocation.address1}</p>
                  {supplierLocation.address2 && (
                    <p>{supplierLocation.address2}</p>
                  )}
                  <p>
                    {supplierLocation.city}, {supplierLocation.state},{" "}
                    {supplierLocation.country} - {supplierLocation.postalCode}
                  </p>
                  <p>
                    <span className="font-medium">GSTIN:</span>{" "}
                    {supplierLocation.gstin}
                  </p>
                </div>
              ) : (
                <p></p>
          ...
          <div className="border-[1.5px] h-full rounded-lg border-neutral-200 text-sm">
            ...
            <form className="space-y-2 px-4 py-3 ">
              ...
              <div className="grid md:grid-cols-2 gap-2">
                ...
                <div className="space-y-2">
                  <Label className={labelClasses} htmlFor="enquiryNumber">
                    Enquiry Number{" "}
                    <span className="text-[#F53D6B] -mr-2">*</span>
                  </Label>
                  <Input
                    className={`${inputClasses} ${
                      errors.enquiryNumber ? "border-red-500" : ""
                    }`}
                    ...
                    onChange={(e) => {
                      ...
                      clearFieldError("enquiryNumber");
                    }}
                  />
                  {errors.enquiryNumber && (
                    <p className="text-red-500 text-xs">
                      {errors.enquiryNumber}
                    </p>
                  )}
                ...
                <div className="space-y-2">
                  <Label className={labelClasses} htmlFor="enquiryDate">
                    Enquiry Date <span className="text-[#F53D6B] -mr-2">*</span>
                  </Label>
                  <InputDatePicker
                    name="enquiryDate"
                    onChange={(dateString) => {
                      setEnquiryDate(dateString);
                      clearFieldError("enquiryDate");
                    }}
                  />
                  {errors.enquiryDate && (
                    <p className="text-red-500 text-xs">{errors.enquiryDate}</p>
                  )}
                </div>
              ...
              <div className="space-y-2">
                <div className="w-full space-y-1">
                  ...
                  <Input
                    className={`${inputClasses} ${
                      errors.kindAttention ? "border-red-500" : ""
                    }`}
                    placeholder="Kind Attention"
                    name="kindAttention"
                    value={kindAttention}
                    onChange={(e) => {
                      setKindAttention(e.target.value);
                      clearFieldError("kindAttention");
                    }}
                  />
      ...
      <div className="flex mt-8 md:mt-10 lg:mt-16 justify-between flex-col md:flex-row gap-5">
        ...
        <div className="space-y-3 text-xs md:text-sm w-full max-w-84">
          ...
          >
            {isSubmitting ? "Submitting..." : "Submit Purchase Quotation"}
          </Button>
        </div>
      </div>
      ...
      <ShowLocations
        isOpen={isSupplierLocationModalOpen}
        onClose={() => setIsSupplierLocationModalOpen(false)}
        onSelectLocation={handleSelectSupplierLocation}
