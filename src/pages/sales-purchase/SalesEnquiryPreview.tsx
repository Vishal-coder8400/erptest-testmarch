...
const SalesEnquiryPreview: React.FC = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    // }
    get(`/inventory/sales-inquiry/${id}`)
      .then((data) => {
        // localStorage.setItem("User", JSON.stringify(data?.data));
  ...
  useEffect(() => {
    if (purchaseOrder) {
      ...
      const tax = items.reduce((acc: number, item: any) => {
        return acc + ((Number(item.tax) * Number(item.totalAmount)) / 100 || 0);
      }, 0);
    ...
    <div className="min-h-screen pb-16">
      ...
      <div className="px-10 mt-10 text-xs flex justify-center items-center">
        <div className="w-full max-w-6xl shadow space-y-2 p-4">
          ...
          <PurchaseOrderPreviewTable
            items={itemData.reduce((prev, current) => {
              return [
                ...prev,
                {
                  ...current,
                  totalPrice:
                    Number(current.totalAmount || 0) + Number(current.tax || 0),
                },
              ];
            }, [])}
            inModal={false}
...
export default SalesEnquiryPreview;
